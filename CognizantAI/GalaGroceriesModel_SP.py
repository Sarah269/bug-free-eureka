# Gala Groceries Regression Model for predicting stock levels
...
def train_algorithm_with_cross_validation(
    X: pd.DataFrame = None, 
    y: pd.Series = None, K: int = 10, SPLIT: float = 0.4
):
    """
    ...
    :parm       SPLIT: float, train/test split
...
def main():
    ...
    # raise an exception if target is not present in dataframe
    X,y = create_target_and_predictors(df, target="estimated_stock_pct")
    ...
    # Print average MAE
    train_algorithm_with_cross_validation(X,y,10,0.4)
